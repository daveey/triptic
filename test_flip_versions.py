"""
Test to verify flip uses the same version that's displayed on the page.
"""
...
def test_flip_versions():
    """Test that flip uses the currently displayed version."""
    ...
    with sync_playwright() as p:
        ...
        print("=" * 80)
        print("TESTING FLIP VERSION CONSISTENCY")
        print("=" * 80)

        # Test cat asset group
        ...
        # Check which version was flipped (should be in server logs)
        # For now, just verify the flip succeeded
        if flip_requests:
        ...
        # Take screenshot
        page.screenshot(path='/tmp/cat_flip_test.png', full_page=True)
        print(f"\n   Screenshot: /tmp/cat_flip_test.png")
        ...
        print("\n" + "=" * 80)
        print(f"DISPLAYED: {displayed_uuid}")
